<div id="sidebar-wrapper" data-simplebar="" data-simplebar-auto-hide="true">
    <div class="brand-logo">
        <a href="dashboard.php">
            <img src="../assets/images/logo.svg" class="w-50" alt="logo icon">
        </a>
    </div>

    <ul class="sidebar-menu do-nicescrol">
        <li class="sidebar-header">GENERAL</li>
        <li>
            <a href="dashboard.php">
                <i class="zmdi zmdi-view-dashboard" aria-hidden="true"></i><span>Dashboard</span>
            </a>
        </li>
        <li>
            <a href="resources.php">
                <i class="zmdi zmdi-graduation-cap" aria-hidden="true"></i><span>Resources</span>
            </a>
        </li>
        <li>
            <a href="conversation.php">
                <i class="zmdi zmdi-comments" aria-hidden="true"></i> <span>Chatbot</span>
            </a>
        </li>

        <li class="sidebar-header">ACCOUNT</li>
        <li>
            <a href="profile.php">
                <i class="zmdi zmdi-account" aria-hidden="true"></i><span>Profile Details</span>
            </a>
        </li>
        <li>
            <a href="dependent.php">
                <i class="zmdi zmdi-accounts-list-alt" aria-hidden="true"></i><span>Dependents</span>
            </a>
        </li>
        <li>
            <a href="health-metrics.php">
                <i class="zmdi zmdi-hospital" aria-hidden="true"></i> <span>Health Metric</span>
            </a>
        </li>    
        
        <li class="sidebar-header">APPOINTMENTS</li>
        <li>
            <a href="appointment.php">
                <i class="zmdi zmdi-calendar" aria-hidden="true"></i> <span>Listing</span>
            </a>
        </li>
        <li>
            <a href="doctors.php">
                <i class="fa fa-user-md" aria-hidden="true"></i><span>Find A Doctor</span>
            </a>
        </li>
    </ul>
</div>