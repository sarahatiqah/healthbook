...
if (isset($_SESSION['doctorId'], $_SESSION['password'])) {
  ...
  $returnUrl = isset($_GET['returnUrl']) ? $_GET['returnUrl'] : 'appointment.php';
  ...
  if (isset($_POST['save'])) {
    ...
    $formattedTimeEmail = date('h:i A', strtotime($appTime));
    ...
    if (mysqli_num_rows($checkResult) > 0) {
      ...
      header("Location: edit-appointment.php?id=" . urlencode($id) . "&appDate=" . urlencode($appDate) . "&returnUrl=" . urlencode($returnUrl));
      exit;
    }
    ...
    if (mysqli_query($con, $updateQuery)) {
      ...
      $_SESSION['prompt'] = "Appointment has been changed and email already sent to $patientEmail.";
      ...
      header("Location: edit-appointment.php?id=" . urlencode($id) . "&returnUrl=" . urlencode($returnUrl));
      exit;
    }
  }
  ...
  $idapp = $_GET['id'];
  $query = "SELECT a.*,b.patientName,b.patientPhone,b.patientEmail, c.name_dependent,a.doctorID,d.doctorName,e.name_specialization,e.id_specialization FROM appointment a 
          JOIN patient b 
  ...
  if ($result = mysqli_query($con, $query)) {
    ...
    // Set the default value for appTime
    // $appTime = isset($_POST['appTime']) ? $_POST['appTime'] : $appTime;
    // $appDate = (isset($_GET['appDate']) && $_GET['appDate'] !== null) ? $_GET['appDate'] : date('Y-m-d');

    // $appTime = isset($_POST['appTime']) ? $_POST['appTime'] : $appTime;
    // $appDate = isset($_POST['appDate']) ? $_POST['appDate'] : $appDate;
?>
      ...
      <div id="wrapper">
        ...
        <script>
          function validateForm() {
            // Check if at least one radio button with the name 'appTime' is checked
            if (!document.querySelector('input[name="appTime"]:checked')) {
